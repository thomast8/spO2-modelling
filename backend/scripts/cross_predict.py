"""
Cross-prediction test: fit on one hold, predict all others.

Tests whether model parameters generalise:
  - Within-type: FL#6 -> FL#1, FRC#5 -> FRC#2, RV#4 -> RV#3
  - Cross-type: FL#6 -> FRC/RV, etc.

Mechanistic models (Production, CO2-Bohr) share physiological parameters that
should transfer. Richards parameters are purely curve-specific and should not.

Usage:
    cd backend && uv run python scripts/cross_predict.py
"""
...
# ── Model functions (copied from compare_new_model.py) ─────────────────────
...
def fit_model(model_name, hold):
    ...
    # Weighted SSE for mechanistic, plain SSE for Richards
...
def main():
    ...
    fitted = {}  # (model_name, hold_id) -> params
        ...
        for train_hold in holds:
            ...
            for pred_hold in holds:
                ...
                if train_hold["id"] == pred_hold["id"]:
                    row += f" {'['+f'{r2:.4f}'+']':>10s}"  # self-fit in brackets
                else:
                    row += f" {r2:>10.4f}"
            print(row)
    ...
    print(f"{'='*90}")
    header = f"  {'Pair':<18s}" + "".join(f" {m:>12s}" for m in MODELS)
    print(header)
    print(f"  {'-'*18}" + "".join(f" {'-'*12}" for _ in MODELS))
    ...
    # Cross-type: train on FL#6, predict everything
    print(f"\n{'='*90}")
    print("CROSS-TYPE TRANSFER (trained on FL #6)")
    print(f"{'='*90}")
    header = f"  {'Predict':<16s}" + "".join(f" {m:>12s}" for m in MODELS)
    print(header)
    print(f"  {'-'*16}" + "".join(f" {'-'*12}" for _ in MODELS))
    ...
    for pred_hold in holds:
        ...
        for model_name in MODELS:
            ...
            row += f" {r2:>12.4f}{marker}"
        print(row)

    # Plot: within-type cross-predictions
    fig, axes = plt.subplots(3, 2, figsize=(16, 12))
    colors = {"Production": "#1f77b4", "CO2-Bohr": "#ff7f0e", "Richards": "#2ca02c"}
    ...
    for row_idx, (_, train_id, pred_id, label) in enumerate(pairs[:3]):
        ...
        for model_name in MODELS:
            ...
            ax.plot(pred_hold["t"], pred, color=colors[model_name], lw=1.5, alpha=0.8,
                    label=f"{model_name} (R²={r2:.3f})")
        ax.set_title(f"Predict {hold_labels[pred_id]} (trained on {hold_labels[train_id]})",
        ...
        for model_name in MODELS:
            ...
            ax.plot(train_hold["t"], pred, color=colors[model_name], lw=1.5, alpha=0.8,
                    label=f"{model_name} (R²={r2:.3f})")
        ax.set_title(f"Self-fit on {hold_labels[train_id]}", fontsize=11, fontweight="bold")
        ax.set_xlabel("Time (s)")
        ax.set_ylabel("SpO2 (%)")
        ax.set_ylim(30, 105)
        ax.legend(fontsize=8, loc="lower left")
    ...
    plt.tight_layout()
    out = Path(__file__).resolve().parent / "cross_predict.png"
