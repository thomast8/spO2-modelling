"""
v7 Experiment 04: Minimal Identifiable Sensor Model.

v7.03 broke the gain/offset/clipping degeneracy (r_offset: -8 -> 0.24, b_s: 1.56 -> 1.09,
tau_0: 9 -> 34, gamma freed to 1.07). The no-clip-in-loss fix worked exactly as designed.

But the fix exposed three new failure modes:
  1. Covariate sponge: beta_g=-2.0 at bound, beta_s=2.0 at bound. Covariates computed
     from the lag-distorted SpO2 trace make the prediction circular.
  2. m_h / tau_0 degeneracy: m_h=-15 at bound for 3/5 holds. Observed nadir timing is
     ~t_end + m_h + tau_0, so any pair summing to the same value gives similar loss.
  3. 18% saturation: S_start ranges 91.7-98.8 while observed plateaus are ~97-99.

v7.04 changes:
  1. Remove lag covariates (beta_g = beta_s = 0): circular prediction removed.
     Lag model: eff_lag = tau_0 + delta_h.
  2. Constrain m_h to [0, 40]: nadir cannot occur before apnea ends.
     Widen delta bounds to [-20, 20] as insurance.
  3. Baseline anchor prior on S_start: S_base_h = median(spo2[t <= 20]),
     prior 0.5 * (S_start - S_base_h)^2 per hold.
  4. Flag RV#4 as out-of-regime: apnea-window nadir before t_end-2s.
     Keep in training, report with flag.
  5. LOHO held-out: switch to DE (popsize=20, maxiter=500).
     Non-smooth objective makes L-BFGS-B unreliable.
  6. Stage B: frozen-lag vs weak-lag diagnostic. Two modes reported.

Carries forward from v7.03:
  No-clip-in-loss, Student-t NLL, Huber timing, LOHO-Inference, saturation diagnostic,
  Stage B apnea-window nadir, DE popsize=40/maxiter=4000, fixed cv=0.15.

Usage:
    cd backend && uv run python -u scripts/experiments/exp_v7_04/exp_v7_04_minimal.py
"""
...
LAMBDA_M = 0.0078          # N(0, 8): 1/(2*8^2) = 0.0078 (loosened for observed nadir shifts)
LAMBDA_OFFSET = 0.125      # N(0, 2): 1/(2*2^2) = 0.125
LAMBDA_GAIN = 50.0         # N(1, 0.1): 1/(2*0.1^2) = 50
LAMBDA_NADIR = 500.0       # Huber timing penalty (delta=8s) (structural, not a prior)
LAMBDA_S_BASE = 0.5        # Baseline anchor: ~N(S_base, 1.0): 1/(2*1^2) = 0.5
CV_FIXED = 0.15            # Fixed kernel shape

# ── Stage B: Physiology regularization (MAP-correct: lambda = 1/(2*sigma^2))

LAMBDA_PVO2 = 0.02         # N(25, 5): 1/(2*5^2) = 0.02
...
OFFSET_BOUNDS = (-3, 3)        # r_offset (a_s) — tightened from (-8,8), physical constraint
GAIN_BOUNDS = (0.5, 2.0)       # b_s gain
DELTA_BOUNDS = (-20, 20)       # per-hold residual shifts (widened from [-15,15])
M_H_BOUNDS = (0, 40)           # per-hold nadir shift (non-negative: nadir cannot precede apnea end)
...
def run_stage_a(fit_holds, nadir_info, s_base_values):
    ...
    Parameter layout (28 params for 5 holds):
    ...
    print(f"  Lag model: eff_lag = tau_0 + delta_h (no covariates)")
    print(f"  Fixed: cv={CV_FIXED}")
    print(f"  Loss: Student-t NLL (nu={NU_STUDENT}, sigma={SIGMA_STUDENT})")
    print(f"  S_base anchors: {[f'{s_base_values[h['id']]:.1f}' for h in fit_holds]}")
    ...
    def objective(flat):
        tau_0, r_offset, b_s = flat[:n_global]
        ...
        for i, h in enumerate(fit_holds):
            ...
            # Apply gamma kernel with effective lag (no covariates)
            eff_lag = max(tau_0 + deltas[i], 1.0)
            ...
            # No clip for loss - plateau must be informative
            ...
            # S_start baseline anchor prior
            total += LAMBDA_S_BASE * (S_start - s_base_values[h["id"]]) ** 2
        ...
        # m_h: N(0, 8) prior
        total += LAMBDA_M * np.sum(m_shifts**2)
...
def evaluate_stage_a(flat_a, fit_holds, nadir_info):
    ...
    tau_0, r_offset, b_s = flat_a[:n_global]
...
def run_stage_b(fit_holds, nadir_info, frozen_sensor, label="Stage B"):
...
def evaluate_stage_b(flat_b, fit_holds, nadir_info,
    ...
    for h in target_holds:
        ...
        # Apnea-window nadir: model is apnea-only, so restrict to [0, t_end+5]
...
def _loho_worker(args):
    ...
    Phase 1: Build bounds for globals + train-only params. Run DE on train holds.
    Phase 2: Extract fitted globals. Optimize 4 held-out latent params (m_h,
             S_start, S_min, v_up) via DE against held-out data.

    This tests: "do global sensor params transfer to unseen holds?"
    """
    ...
    bounds = [TAU0_BOUNDS, OFFSET_BOUNDS, GAIN_BOUNDS]
    n_global = len(bounds)
    ...
    # ── Phase 2: Optimize held-out latent params via DE ──
    tau_0_fit, r_off_fit, b_s_fit = res.x[:n_global]

    # No delta for held-out hold (that's the test)
    eff_lag_ho = max(tau_0_fit, 1.0)
    ...
    # 4D optimization: [m_h, S_start, S_min, v_up]
    ho_bounds = [M_H_BOUNDS, S_START_BOUNDS, S_MIN_BOUNDS, V_UP_BOUNDS]

    def ho_objective(x):
        m_h, S_start, S_min, v_up = x
        latent = build_piecewise_linear(t_1hz_ho, left_out["t_end"], S_start, S_min, v_up, m_h)
        filtered = apply_gamma_kernel(latent, eff_lag_ho, CV_FIXED)
        pred_1hz = r_off_fit + b_s_fit * filtered
        ...
        total += LAMBDA_M * m_h ** 2  # m_h prior
        total += LAMBDA_S_BASE * (S_start - s_base_values[left_out["id"]]) ** 2
        return total
    ...
    ho_m_h, ho_S_start, ho_S_min, ho_v_up = ho_res.x
    ...
    # ── Phase 2b: LOHO-Inference (5D: m_h, S_start, S_min, v_up, delta_ho) ──
    # Tests: "do globals + inferred delta transfer to unseen holds?"

    ho_bounds_inf = [M_H_BOUNDS, S_START_BOUNDS, S_MIN_BOUNDS, V_UP_BOUNDS, DELTA_BOUNDS]

    def ho_inference_objective(x):
        m_h, S_start, S_min, v_up, delta_ho = x
        latent = build_piecewise_linear(t_1hz_ho, left_out["t_end"], S_start, S_min, v_up, m_h)
        eff_lag_inf = max(tau_0_fit + delta_ho, 1.0)
    ...
    inf_m_h, inf_S_start, inf_S_min, inf_v_up, inf_delta = ho_res_inf.x
    eff_lag_inf = max(tau_0_fit + inf_delta, 1.0)

    latent_inf = build_piecewise_linear(
        t_1hz_ho, left_out["t_end"], inf_S_start, inf_S_min, inf_v_up, inf_m_h)
    filtered_inf = apply_gamma_kernel(latent_inf, eff_lag_inf, CV_FIXED)
    pred_inf_1hz = r_off_fit + b_s_fit * filtered_inf
...
def run_profile_likelihood(fit_holds, nadir_info, s_base_values,
    ...
    args_list = [(tau_val, fit_holds, nadir_info, s_base_values)
    ...
    for tau_val, result in raw_results:
        ...
        print(f"    tau_0={tau_val:5.1f}: loss={result['loss']:.2f}, "
              f"r_offset={r_off:.3f}, b_s={b_s:.3f}", flush=True)
...
def _sensitivity_worker(args):
    ...
    tau_val, fit_holds, nadir_info, frozen_sensor_template = args
    frozen = dict(frozen_sensor_template)
    frozen["tau_0"] = tau_val
    flat_b, success = run_stage_b(fit_holds, nadir_info, frozen, label="Sensitivity")
    return tau_val, flat_b


def compute_sensitivity(profile_results, fit_holds, nadir_info,
                        frozen_sensor_template):
...
def plot_stage_a_detail(eval_a, latent_curves, all_holds, nadir_info, output_path):
    ...
    fig.suptitle("v7.04 Stage A: Piecewise-Linear Latent + Gamma Kernel",
...
def plot_stage_b_detail(eval_b, all_holds, nadir_info, frozen_sensor, output_path):
    ...
    fig.suptitle("v7.04 Stage B: Apnea-Only Physiology (frozen sensor)",
                 fontsize=14, fontweight="bold")
    plt.tight_layout()
    plt.savefig(output_path, dpi=150, bbox_inches="tight")
...
def plot_loho_summary(loho_results, output_path):
    ...
    fig.suptitle("v7.04 Stage A: Leave-One-Hold-Out Cross-Validation",
                 fontsize=14, fontweight="bold")
    plt.tight_layout()
    plt.savefig(output_path, dpi=150, bbox_inches="tight")
...
def plot_profile_likelihood(profile_results, output_path):
    ...
    fig.suptitle("v7.04: tau_0 Profile Likelihood (Stage A)",
                 fontsize=14, fontweight="bold")
    plt.tight_layout()
    plt.savefig(output_path, dpi=150, bbox_inches="tight")
...
def main():
    print("=" * 120)
    print("v7.04: Minimal Identifiable Sensor Model")
    print("=" * 120)
    ...
    # ── Compute baselines for S_start prior ─────────────────────────────────
    s_base_values = {}
    for h in all_holds:
        plateau_mask = h["t"] <= 20
    ...
    print(f"\n  Baseline anchors (S_base = median SpO2[t<=20]):")
    for h in fit_holds:
        print(f"    {h['type']}#{h['id']}: S_base={s_base_values[h['id']]:.1f}")

    # ── QC flags (out-of-regime detection) ────────────────────────────────────
    qc_flags = {}
    for h in all_holds:
        ni = nadir_info[h["id"]]
        # Flag if apnea-window nadir occurs before t_end - 2s
    ...
    # ── Summary of changes from v7.03 ────────────────────────────────────────
    print(f"\nKey changes from v7.03:")
    print(f"  1. Remove lag covariates (beta_g = beta_s = 0)")
    print(f"  2. Constrain m_h to {M_H_BOUNDS} (non-negative)")
    print(f"  3. Baseline anchor prior on S_start (lambda={LAMBDA_S_BASE})")
    print(f"  4. Flag RV#4 as out-of-regime")
    print(f"  5. LOHO held-out: DE instead of L-BFGS-B")
    print(f"  6. Stage B: frozen-lag vs weak-lag diagnostic")
    print(f"  7. Widen delta bounds: {DELTA_BOUNDS} (was (-15, 15))")

    # ══════════════════════════════════════════════════════════════════════════
    # STAGE A: Sensor-First Calibration (28 params)
    # ══════════════════════════════════════════════════════════════════════════
    print(f"\n{'='*120}")
    print("STAGE A: Sensor-First Calibration (28 params)")
    print(f"{'='*120}")
    ...
    n_global = 3
    global_names = ["tau_0", "r_offset", "b_s"]
    global_bounds = [TAU0_BOUNDS, OFFSET_BOUNDS, GAIN_BOUNDS]
    ...
    tau_0, r_offset_a, b_s_a = flat_a[:n_global]
    ...
    print(f"\n  Stage A -- Per-hold breakdown (base tau_0={tau_0:.2f}):")
    for i, h in enumerate(fit_holds):
        residual = deltas_a[i]
        eff = max(tau_0 + residual, 1.0)
        ...
        print(f"    {h['type']}#{h['id']}: "
              ...
              f"S_start={S_start:.1f} (base={s_base_values[h['id']]:.1f}), "
              f"S_min={S_min:.1f}, v_up={v_up:.2f}{bound_str}{qc_str}")

    eval_a, latent_curves_a = evaluate_stage_a(flat_a, fit_holds, nadir_info)
    ...
    for i, h in enumerate(fit_holds):
        ...
        S_start, S_min, v_up = flat_a[lp_start:lp_start + 3]
        t_1hz = np.arange(0, h["t"][-1] + 1, 1.0)
        latent = build_piecewise_linear(t_1hz, h["t_end"], S_start, S_min, v_up, m_shifts_a[i])
        eff_lag = max(tau_0 + deltas_a[i], 1.0)
        filtered = apply_gamma_kernel(latent, eff_lag, CV_FIXED)
        pred_raw = r_offset_a + b_s_a * filtered
    ...
    flat_b, conv_b = run_stage_b(fit_holds, nadir_info, frozen_sensor, label="Stage B (frozen-lag)")
    ...
    eval_b = evaluate_stage_b(
        flat_b, fit_holds, nadir_info, frozen_sensor, all_holds)
    ...
    ts_results = run_stage_b_time_split(fit_holds, nadir_info, frozen_sensor)
    ...
    profile_results = run_profile_likelihood(
        fit_holds, nadir_info, s_base_values, tau_0_values)
    ...
    sensitivities = compute_sensitivity(
        profile_results, fit_holds, nadir_info, frozen_sensor)
    ...
    # Stage A sponge
    a_param_names = ["tau_0", "r_offset", "b_s"]
    a_bounds = [TAU0_BOUNDS, OFFSET_BOUNDS, GAIN_BOUNDS]
    a_prior_sigmas = {
    ...
    plot_stage_a_detail(eval_a, latent_curves_a, all_holds, nadir_info,
                        output_dir / "exp_v7_04_stage_a.png")

    plot_stage_b_detail(eval_b, all_holds, nadir_info, frozen_sensor,
                        output_dir / "exp_v7_04_stage_b.png")

    plot_loho_summary(loho_results, output_dir / "exp_v7_04_loho.png")

    plot_profile_likelihood(profile_results, output_dir / "exp_v7_04_profile.png")

    plot_sensitivity(sensitivities, output_dir / "exp_v7_04_sensitivity.png")
    ...
    # Build weak-lag frozen sensor: widen delta bounds centered on Stage A values
    frozen_sensor_wl = dict(frozen_sensor)
    # The deltas in frozen_sensor are from Stage A; for weak-lag we let Stage B
    # re-fit them with wider bounds centered on Stage A values.
    # The run_stage_b function already re-fits deltas, so we just use wider bounds.
    # We need a modified run that uses prior on deltas centered on Stage A values.
    ...
    # Weak-lag Stage B: same as frozen-lag but with delta prior centered on Stage A
    def run_stage_b_weak_lag(fit_holds, nadir_info, frozen_sensor, delta_a_values):
        ...
        # Weak-lag prior: N(delta_stageA, 3) per delta
        ...
        def objective(flat):
            ...
            # Weak-lag prior: N(delta_stageA, 3) instead of shrinkage toward 0
            for i in range(n_holds):
    ...
    # Check convergence: physiology within 20%
    max_phys_diff = max(
    ...
    # 1. r_offset near zero
    print(f"\n  1. r_offset={r_offset_a:.4f} in (-2, 2): "
          f"{'PASS' if abs(r_offset_a) < 2 else 'FAIL'}")

    # 2. b_s near 1.0
    print(f"  2. b_s={b_s_a:.4f} in (0.8, 1.2): "
          f"{'PASS' if 0.8 < b_s_a < 1.2 else 'FAIL'}")

    # 3. tau_0 in [15, 30]
    print(f"  3. tau_0={tau_0:.1f} in [15, 30]: "
          f"{'PASS' if 15 <= tau_0 <= 30 else 'FAIL'}")

    # 4. m_h values all >= 0
    all_mh_nonneg = all(m >= -0.01 for m in m_shifts_a)
    print(f"  4. m_h all >= 0: {['%.1f' % m for m in m_shifts_a]}: "
          f"{'PASS' if all_mh_nonneg else 'FAIL'}")

    # 5. Saturation < 5%
    print(f"  5. Saturation: {pct_total:.1f}% (target < 5%): "
          f"{'PASS' if pct_total < 5 else 'FAIL'}")

    # 6. No deltas at bounds
    deltas_at_bound = sum(1 for d in deltas_a if is_at_bound(d, *DELTA_BOUNDS))
    print(f"  6. Deltas at bounds: {deltas_at_bound}/5: "
          f"{'PASS' if deltas_at_bound == 0 else 'FAIL'}")

    # 7. LOHO timing error
    print(f"  7. LOHO |timing error| avg={avg_loho_nerr:.1f}s (target < 5s): "
          f"{'PASS' if avg_loho_nerr < 5 else 'FAIL'}")

    # 8. Profile depth > 15%
    losses_sorted = sorted(losses)
    profile_depth = (losses_sorted[-1] - losses_sorted[0]) / losses_sorted[0] * 100 if losses_sorted[0] > 0 else 0
    print(f"  8. Profile depth={profile_depth:.1f}% (target > 15%): "
          f"{'PASS' if profile_depth > 15 else 'FAIL'}")

    # 9. Profile non-monotone
    print(f"  9. Profile non-monotone: {'PASS' if not is_monotone else 'FAIL (monotone)'}")
    print(f"     Minimum at tau_0={min_tau:.1f}")

    # 10. Frozen-lag vs weak-lag convergence
    print(f"  10. Frozen vs weak-lag divergence={max_phys_diff:.1f}% (target < 20%): "
          f"{'PASS' if wl_converged else 'FAIL'}")

    # 11. Gamma interior
    gamma_b = flat_b[2]
    gamma_interior = not is_at_bound(gamma_b, *GAMMA_BOUNDS)
    print(f"  11. gamma={gamma_b:.4f} interior: "
          f"{'PASS' if gamma_interior else 'FAIL (at bound)'}")

    # 12. Params at bounds
    total_at_bound = at_bound_a + at_bound_b
    print(f"  12. Global params at bounds: {total_at_bound}")

    # 13. QC-passing averages
    ...
    if qc_passing:
        ...
        print(f"  13. QC-passing avg R2n={qc_r2n:.4f}, |timing|={qc_nerr:.1f}s")
