"""
v7 Experiment 03: Break the Plateau-Clipping Degeneracy.

v7.02 fixed priors, LOHO, and nadir metrics. But the corrected (weaker) priors
exposed a 3-way degeneracy: r_offset=-8.0, b_s=1.56, tau_0=9.3. These compensate
each other because the hard clip at 100 makes the plateau uninformative. Any
(r_offset, b_s) that pushes pred_raw > 100 gives the same clipped loss.

v7.03 changes:
  1. Remove clip from loss: compute Student-t NLL on unclipped predictions.
     Keep clip only for nadir timing penalty, plotting, and display.
     This makes the plateau informative: S_start bounds [90,100] force
     b_s ~ 1.0 and r_offset ~ 0, breaking the degeneracy.
  2. LOHO-Inference mode: Phase 2b with delta optimized for held-out hold.
     Distinguishes "covariates can't predict timing" from "model can't fit shape."
  3. Saturation diagnostic: count % of samples where pred_raw > 100 per hold.
  4. Stage B apnea-window observed nadir: use min in [0, t_end+5] instead of
     the wide-window nadir from compute_nadir_info().
  5. Tighten r_offset bounds: (-8,8) -> (-3,3). Physical constraint: calibrated
     finger pulse oximeters can't have 8-point systematic offset.

Carries forward from v7.02:
  MAP-correct priors, two-phase LOHO, robust nadir, threshold time-split,
  DE popsize=40/maxiter=4000, fixed cv=0.15, Student-t NLL.

Usage:
    cd backend && uv run python -u scripts/experiments/exp_v7_03/exp_v7_03_unclip.py
"""
...
LAMBDA_BETA = 0.125        # N(0, 2): 1/(2*2^2) = 0.125
...
M_H_BOUNDS = (-15, 40)         # per-hold nadir shift (widened for observed nadirs up to +35s)
...
def _loho_worker(args):
    """Worker for one LOHO fold. Two-phase: train DE, then L-BFGS-B for held-out.
    ...
    Phase 2: Extract fitted globals. Optimize 4 held-out latent params (m_h,
             S_start, S_min, v_up) via L-BFGS-B against held-out data.
    ...
    # No held-out params in bounds - they were dead dimensions before
    ...
    # ── Phase 2: Optimize held-out latent params via L-BFGS-B ──
    ...
    eff_lag_ho = max(tau_0_fit + beta_g_fit * g_ho + beta_s_fit * neg_s_ho, 1.0)
    ...
    best_result = None
    best_loss = float("inf")
    rng = np.random.RandomState(42 + leave_idx)
    for restart in range(5):
        if restart == 0:
            x0 = np.array([0.0, 97.0, 70.0, 0.5])
        else:
            x0 = np.array([
                rng.uniform(*M_H_BOUNDS),
                rng.uniform(*S_START_BOUNDS),
                rng.uniform(*S_MIN_BOUNDS),
                rng.uniform(*V_UP_BOUNDS),
            ])
        try:
            ho_res = minimize(
                ho_objective, x0, method="L-BFGS-B", bounds=ho_bounds,
                options={"maxiter": 200, "ftol": 1e-12},
            )
            if ho_res.fun < best_loss:
                best_loss = ho_res.fun
                best_result = ho_res
        except Exception:
            continue

    ho_m_h, ho_S_start, ho_S_min, ho_v_up = best_result.x
    ...
    g_ho = severities[left_out["id"]]
    neg_s_ho_val = -end_slopes[left_out["id"]]
    ...
    def ho_inference_objective(x):
        ...
        latent = build_piecewise_linear(t_1hz_ho, left_out["t_end"], S_start, S_min, v_up, m_h)
        eff_lag_inf = max(tau_0_fit + beta_g_fit * g_ho + beta_s_fit * neg_s_ho_val + delta_ho, 1.0)
    ...
    best_result_inf = None
    best_loss_inf = float("inf")
    rng_inf = np.random.RandomState(142 + leave_idx)
    for restart in range(5):
        if restart == 0:
            x0 = np.array([ho_m_h, ho_S_start, ho_S_min, ho_v_up, 0.0])
        else:
            x0 = np.array([
                rng_inf.uniform(*M_H_BOUNDS),
                rng_inf.uniform(*S_START_BOUNDS),
                rng_inf.uniform(*S_MIN_BOUNDS),
                rng_inf.uniform(*V_UP_BOUNDS),
                rng_inf.uniform(*DELTA_BOUNDS),
            ])
        try:
            ho_res_inf = minimize(
                ho_inference_objective, x0, method="L-BFGS-B", bounds=ho_bounds_inf,
                options={"maxiter": 200, "ftol": 1e-12},
            )
            if ho_res_inf.fun < best_loss_inf:
                best_loss_inf = ho_res_inf.fun
                best_result_inf = ho_res_inf
        except Exception:
            continue

    inf_m_h, inf_S_start, inf_S_min, inf_v_up, inf_delta = best_result_inf.x
    eff_lag_inf = max(tau_0_fit + beta_g_fit * g_ho + beta_s_fit * neg_s_ho_val + inf_delta, 1.0)
...
def plot_stage_a_detail(eval_a, latent_curves, all_holds, nadir_info, output_path):
    ...
    fig.suptitle("v7.03 Stage A: Piecewise-Linear Latent + Gamma Kernel",
...
def plot_stage_b_detail(eval_b, all_holds, nadir_info, frozen_sensor, output_path):
    ...
    fig.suptitle("v7.03 Stage B: Apnea-Only Physiology (frozen sensor)",
                 fontsize=14, fontweight="bold")
    plt.tight_layout()
    plt.savefig(output_path, dpi=150, bbox_inches="tight")
...
def plot_loho_summary(loho_results, output_path):
    ...
    fig.suptitle("v7.03 Stage A: Leave-One-Hold-Out Cross-Validation",
                 fontsize=14, fontweight="bold")
    plt.tight_layout()
    plt.savefig(output_path, dpi=150, bbox_inches="tight")
...
def plot_profile_likelihood(profile_results, output_path):
    ...
    fig.suptitle("v7.03: tau_0 Profile Likelihood (Stage A)",
                 fontsize=14, fontweight="bold")
    plt.tight_layout()
    plt.savefig(output_path, dpi=150, bbox_inches="tight")
...
def main():
    print("=" * 120)
    print("v7.03: Break the Plateau-Clipping Degeneracy")
    print("=" * 120)
    ...
    # ── Summary of changes from v7.02 ────────────────────────────────────────
    print(f"\nKey changes from v7.02:")
    print(f"  1. Remove clip from loss: NLL on raw predictions (plateau informative)")
    print(f"  2. LOHO-Inference: Phase 2b with delta for held-out hold")
    print(f"  3. Saturation diagnostic: % samples with pred_raw > 100")
    print(f"  4. Stage B apnea-window observed nadir: min in [0, t_end+5]")
    print(f"  5. Tighten r_offset bounds: {OFFSET_BOUNDS} (was (-8, 8))")
    ...
    for i, h in enumerate(fit_holds):
        ...
        shift = beta_g_a * g_h_list[i] + beta_s_a * neg_s_h_list[i] + deltas_a[i]
        eff_lag = max(tau_0 + shift, 1.0)
        filtered = apply_gamma_kernel(latent, eff_lag, CV_FIXED)
    ...
    plot_stage_a_detail(eval_a, latent_curves_a, all_holds, nadir_info,
                        output_dir / "exp_v7_03_stage_a.png")

    plot_stage_b_detail(eval_b, all_holds, nadir_info, frozen_sensor,
                        output_dir / "exp_v7_03_stage_b.png")

    plot_loho_summary(loho_results, output_dir / "exp_v7_03_loho.png")

    plot_profile_likelihood(profile_results, output_dir / "exp_v7_03_profile.png")

    plot_sensitivity(sensitivities, output_dir / "exp_v7_03_sensitivity.png")
    ...
    print(f"  9. Stage B R2(apnea) avg={avg_r2a_b:.4f} (target >= 0.95): "
          f"{'PASS' if avg_r2a_b >= 0.95 else 'FAIL'}")

    # 10. Profile non-monotone
    ...
    print(f"  11. Params at bounds: {total_at_bound}")
