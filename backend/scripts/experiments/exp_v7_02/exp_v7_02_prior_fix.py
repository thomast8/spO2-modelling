"""
v7 Experiment 02: Prior Calibration + LOHO Fix + Honest Metrics.

v7.02 established the right architecture (two-stage, piecewise-linear latent,
fixed cv=0.15, Student-t NLL) but external review identified three issues:
  1. All priors were 16-6400x tighter than documented (lambda = 1/(2*sigma^2))
  2. LOHO was broken: held-out latent params were dead dimensions in DE
  3. Stage B nadir metric used full trace (model is apnea-only)

v7.02 fixes:
  1. MAP-correct prior coefficients: lambda = 1/(2*sigma^2)
  2. Two-phase LOHO: train DE on train holds, then L-BFGS-B for held-out latent
  3. Apnea-window nadir metric for Stage B
  4. Widened bounds: b_s [0.5, 2.0], m_h [-15, 40]
  5. Increased DE exploration: popsize=40, maxiter=4000
  6. Robust nadir definition: local search near t_end with median smoothing
  7. Threshold-based time-split: SpO2 < 95 trigger instead of fixed fraction
...
Usage:
    cd backend && uv run python -u scripts/experiments/exp_v7_02/exp_v7_02_prior_fix.py
"""
...
GAIN_BOUNDS = (0.5, 2.0)       # b_s gain (widened, was pinned at 1.2 in v7.02)
...
def evaluate_stage_b(flat_b, fit_holds, nadir_info, severities, end_slopes,
    ...
    for h in target_holds:
        ...
        nadir_spo2_obs = nadir_info[h["id"]]["spo2_nadir"]
...
def _loho_worker(args):
    ...
    def ho_objective(x):
        ...
        total += nadir_timing_penalty_huber(left_out["t"][m], pred_at_obs[m], nadir_t_ho)
    ...
    # Predict held-out hold with optimized latent params
...
def plot_stage_a_detail(eval_a, latent_curves, all_holds, nadir_info, output_path):
    ...
    fig.suptitle("v7.02 Stage A: Piecewise-Linear Latent + Gamma Kernel",
...
def plot_stage_b_detail(eval_b, all_holds, nadir_info, frozen_sensor, output_path):
    ...
    fig.suptitle("v7.02 Stage B: Apnea-Only Physiology (frozen sensor)",
                 fontsize=14, fontweight="bold")
    plt.tight_layout()
    plt.savefig(output_path, dpi=150, bbox_inches="tight")
...
def plot_loho_summary(loho_results, output_path):
    ...
    fig.suptitle("v7.02 Stage A: Leave-One-Hold-Out Cross-Validation",
                 fontsize=14, fontweight="bold")
    plt.tight_layout()
    plt.savefig(output_path, dpi=150, bbox_inches="tight")
...
def plot_profile_likelihood(profile_results, output_path):
    ...
    fig.suptitle("v7.02: tau_0 Profile Likelihood (Stage A)",
                 fontsize=14, fontweight="bold")
    plt.tight_layout()
    plt.savefig(output_path, dpi=150, bbox_inches="tight")
...
def main():
    print("=" * 120)
    print("v7.02: Prior Calibration + LOHO Fix + Honest Metrics")
    print("=" * 120)
    ...
    # ── Summary of changes from v7.01 ────────────────────────────────────────
    print(f"\nKey changes from v7.01:")
    print(f"  1. MAP-correct priors: lambda = 1/(2*sigma^2)")
    print(f"     tau_0: {LAMBDA_TAU0}, beta: {LAMBDA_BETA}, m_h: {LAMBDA_M}, "
          f"r_offset: {LAMBDA_OFFSET}, b_s: {LAMBDA_GAIN}")
    print(f"     PvO2: {LAMBDA_PVO2}, k_CO2: {LAMBDA_K_CO2}, PaCO2: {LAMBDA_PACO2}, "
          f"gamma: {LAMBDA_GAMMA}")
    print(f"  2. Two-phase LOHO: train DE + L-BFGS-B for held-out latent")
    print(f"  3. Apnea-window nadir metric for Stage B")
    print(f"  4. Widened bounds: b_s {GAIN_BOUNDS}, m_h {M_H_BOUNDS}")
    print(f"  5. Increased DE: popsize=40, maxiter=4000")
    print(f"  6. Robust nadir: local search near t_end with median smoothing")
    print(f"  7. Threshold time-split: SpO2 < 95 trigger")
    ...
    plot_stage_a_detail(eval_a, latent_curves_a, all_holds, nadir_info,
                        output_dir / "exp_v7_02_stage_a.png")

    plot_stage_b_detail(eval_b, all_holds, nadir_info, frozen_sensor,
                        output_dir / "exp_v7_02_stage_b.png")

    plot_loho_summary(loho_results, output_dir / "exp_v7_02_loho.png")

    plot_profile_likelihood(profile_results, output_dir / "exp_v7_02_profile.png")

    plot_sensitivity(sensitivities, output_dir / "exp_v7_02_sensitivity.png")
