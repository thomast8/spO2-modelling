"""
v4 partial-transfer cross-prediction: fix shared physiology, re-fit initial conditions.

Tests whether the delay observation model improves parameter separation into:
  - Shared physiology (same person): pvo2, gamma, k_co2, r_offset, d
  - Type-specific initial conditions: pao2_0, tau_washout, paco2_0

Adds CO2-Bohr+Delay to the partial-transfer test from v3.

Usage:
    cd backend && uv run python -u scripts/cross_predict_partial_v4.py
"""
...
MODELS = {
    ...
    "CO2-Bohr+Delay": {
        ...
        "type_specific": [0, 2, 4],    # pao2_0, tau_washout, paco2_0
        "shared": [1, 3, 5, 6, 7],     # pvo2, gamma, k_co2, r_offset, d
    },
}
...
def main():
    ...
    for model_name in MODELS:
        ...
        print(
            f"\n  {'Source → Target':<22s}  {'Raw R²':>8s}  {'Partial R²':>10s}  "
            f"{'Self-fit R²':>11s}  {'Raw RMSE':>9s}  {'Part RMSE':>9s}  {'Self RMSE':>9s}  "
            f"{'Partial params':s}"
        )
        print(
            f"  {'-'*22}  {'-'*8}  {'-'*10}  {'-'*11}  {'-'*9}  {'-'*9}  {'-'*9}  {'-'*40}"
        )
        ...
        for source_hold in holds:
            for target_hold in holds:
                ...
                print(
                    f"\r  {pair_label:<22s}  {raw_r2:>8.4f}  {partial_r2:>10.4f}  "
                    f"{self_r2:>11.4f}  {raw_rmse:>9.2f}  {partial_rmse:>9.2f}  "
                    f"{self_rmse:>9.2f}  {param_str}",
                    flush=True,
                )

    # ── Step 3: Cross-type summary ──────────────────────────────────────────
    ...
    for model_name in MODELS:
        ...
        for src_id, tgt_id in cross_type_pairs:
            ...
            print(
                f"  {hold_labels[src_id]+' → '+hold_labels[tgt_id]:<22s}  "
                f"{raw_r2:>8.4f}  {partial_r2:>8.4f}  {self_r2:>8.4f}  {recovery:>7.1f}%"
            )
    ...
    colors = {"CO2-Bohr": "#1f77b4", "CO2-Bohr+Delay": "#ff7f0e"}
    ...
    for idx, (src_id, tgt_id, title) in enumerate(plot_pairs):
        ...
        for model_name in MODELS:
            ...
            ax.plot(
                tgt["t"], raw_pred, color=colors[model_name], lw=1, alpha=0.4,
                ls="--", label=f"{model_name} raw (R²={raw_r2:.3f})",
            )
            ax.plot(
                tgt["t"], partial_pred, color=colors[model_name], lw=1.5, alpha=0.8,
    ...
    plt.tight_layout()
    out = Path(__file__).resolve().parent / "cross_predict_partial_v4.png"
